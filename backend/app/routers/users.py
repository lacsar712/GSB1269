...
from app.services.auth import get_password_hash, get_current_active_user, get_current_admin_user
...
def update_user(
    ...
    if not is_admin:
        if "role_id" in update_data or "is_active" in update_data:
            raise HTTPException(
                status_code=403,
                detail="非管理员无法修改角色或启用状态"
            )
